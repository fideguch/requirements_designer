...
> ※ 所要時間はプロジェクト規模・ドメインの複雑さ・ユーザーの応答速度により大幅に変動します。複雑なプロジェクトでは記載時間の1.5-2倍かかる場合があります。
...
> **Figma MCP 連携範囲**: Phase 5 でのみ使用。Figma MCP サーバー接続が必要。デザインシステム・WF・モックアップの「Figmaキャンバスへの直接生成」を行う。手動デザインワークフローの代替ではなく、要件からの初期プロトタイプ生成が主目的。詳細は [DESIGN.md](DESIGN.md) を参照。
